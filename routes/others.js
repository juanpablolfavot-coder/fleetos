...
+params.length); }
    if (capacity_l !== undefined) { params.push(capacity_l); fields.push('capacity_l=
...
+params.length); }
    if (!fields.length) return res.status(400).json({ error: 'Nada que actualizar' });
    params.push(req.params.id);
    const r = await query('UPDATE tanks SET '+fields.join(',')+',updated_at=NOW() WHERE id=
...
+params.length+' RETURNING *', params);
    if(!r.rows[0]) return res.status(404).json({error:'Cisterna no encontrada'}); res.json(r.rows[0]);
  }catch(err){res.status(500).json({error:'Error cisterna'});}
});
