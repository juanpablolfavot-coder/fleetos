...
router.post('/', authenticate, requireRole('dueno','gerencia','jefe_mantenimiento'), async (req, res) => {
  try {
    const { notes, sucursal, area, tipo='flota', vehicle_id, iva_pct=0, items=[] } = req.body;
    ...
    const po = await query(`INSERT INTO purchase_orders (code,status,requested_by,sucursal,area,tipo,vehicle_id,notes,iva_pct,total_estimado) VALUES ($1,'borrador',$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,
      [code,req.user.id,sucursal||null,area||null,tipo||'flota',vehicle_id||null,notes||null,parseFloat(iva_pct||0),total]);
...
router.patch('/:id', authenticate, requireRole('dueno','gerencia','jefe_mantenimiento'), async (req, res) => {
  try {
    const { proveedor,factura_nro,factura_fecha,factura_monto,notes,status,sucursal,area,iva_pct,tipo,vehicle_id } = req.body;
